...
public sealed class SecureJournalAppServiceTests
{
    [Fact]
    ...
    [Fact]
    public void AdminCanManageUserRoles_AndRemoveUserFromGroup()
    {
        using var ctx = TestAppContext.Create();
        ctx.LoginAsAdmin();
        ...
        Assert.True(ctx.App.AddUserToRole(new UserRoleMembershipRequest
        {
            UserId = user.UserId,
            Role = AppRole.Auditor
        }));
        Assert.False(ctx.App.AddUserToRole(new UserRoleMembershipRequest
        {
            UserId = user.UserId,
            Role = AppRole.Auditor
        }));

        var usersAfterAdd = ctx.App.GetUsers();
        Assert.Contains(usersAfterAdd, u => u.UserId == user.UserId && u.Roles.Contains(AppRole.Auditor));

        Assert.True(ctx.App.RemoveUserFromRole(new UserRoleMembershipRequest
        {
            UserId = user.UserId,
            Role = AppRole.Auditor
        }));
        Assert.False(ctx.App.RemoveUserFromRole(new UserRoleMembershipRequest
        {
            UserId = user.UserId,
            Role = AppRole.Auditor
        }));

        var usersAfterRemoveRole = ctx.App.GetUsers();
        Assert.Contains(usersAfterRemoveRole, u => u.UserId == user.UserId && u.Roles.Contains(AppRole.ProjectUser) && !u.Roles.Contains(AppRole.Auditor));

        Assert.True(ctx.App.RemoveUserFromGroup(new AssignUserToGroupRequest
        {
            UserId = user.UserId,
            GroupId = group.GroupId
        }));
        Assert.False(ctx.App.RemoveUserFromGroup(new AssignUserToGroupRequest
        {
            UserId = user.UserId,
            GroupId = group.GroupId
        }));

        var usersAfterGroupRemoval = ctx.App.GetUsers();
        Assert.Contains(usersAfterGroupRemoval, u => u.UserId == user.UserId && !u.Groups.Contains(group.Name));
    }
    ...
    public void AdminCanDisableUser_AndDisabledUserCannotSignIn()
    {
        ...
        var user = ctx.App.CreateUser(new CreateUserRequest
        {
            ...
            Role = AppRole.ProjectUser,
            IsLocalAccount = true,
            LocalPassword = "Disabled123!"
        });

        Assert.True(ctx.App.DisableUser(user.UserId));
        Assert.False(ctx.App.DisableUser(user.UserId));

        var users = ctx.App.GetUsers();
        Assert.Contains(users, u => u.UserId == user.UserId && u.IsDisabled);

        var login = ctx.App.TryLocalLogin("disableduser", "Disabled123!");
        Assert.False(login.Success);

        Assert.True(ctx.App.EnableUser(user.UserId));
        Assert.False(ctx.App.EnableUser(user.UserId));

        var usersAfterEnable = ctx.App.GetUsers();
        Assert.Contains(usersAfterEnable, u => u.UserId == user.UserId && !u.IsDisabled);

        var loginAfterEnable = ctx.App.TryLocalLogin("disableduser", "Disabled123!");
        Assert.True(loginAfterEnable.Success);
    }
    ...
    public void AdminCanDeleteUser_AndDeletedUserCannotSignIn()
    {
        ...
        var user = ctx.App.CreateUser(new CreateUserRequest
        {
            ...
            Role = AppRole.ProjectUser,
            IsLocalAccount = true,
            LocalPassword = "Delete123!"
        });

        Assert.True(ctx.App.DeleteUser(user.UserId));
        Assert.DoesNotContain(ctx.App.GetUsers(), u => u.UserId == user.UserId);

        var login = ctx.App.TryLocalLogin("deleteuser", "Delete123!");
        Assert.False(login.Success);
    }
    ...
    public void SeparateServiceScopes_KeepIndependentCurrentUsers()
    {
        ...
        try
        {
            ...
            var logger = NullLogger<SecureJournalAppService>.Instance;

            var appA = CreateService(configuration, sqlite, shared, logger);
            var appB = CreateService(configuration, sqlite, shared, logger);
    ...
    public void SessionCookieToken_RestoresScopedUser_AndExpiredTokenIsRejected()
    {
        ...
        try
        {
            ...
            var loginService = CreateService(configuration, sqlite, shared, logger);
            ...
            var restoredService = CreateService(configuration, sqlite, shared, logger, accessorValid, sessions);
            ...
            var expiredService = CreateService(configuration, sqlite, shared, logger, accessorExpired, sessions);
    ...
    [Fact]
    public void OidcDisabledUser_CannotSignIn()
    {
        var dbPath = BuildUniqueDatabasePath("oidc-disabled-user");
        try
        {
            Guid disabledUserId;
            const string issuer = "https://issuer.example";
            const string subject = "sub-disabled";
            ...
            using (var seed = TestAppContext.Create(existingDatabasePath: dbPath, deleteOnDispose: false))
            {
                ...
                var created = seed.App.CreateUser(new CreateUserRequest
                {
                    Username = "oidc-disabled",
                    DisplayName = "OIDC Disabled",
                    Role = AppRole.ProjectUser,
                    IsLocalAccount = false
                });
                disabledUserId = created.UserId;
            }

            var first = CreateOidcService(dbPath, CreateOidcPrincipal("oidc-disabled", issuer, subject, AppRole.ProjectUser));
            Assert.Equal("oidc-disabled", first.GetCurrentUser().Username);

            using (var admin = TestAppContext.Create(existingDatabasePath: dbPath, deleteOnDispose: false))
            {
                admin.LoginAsAdmin();
                Assert.True(admin.App.DisableUser(disabledUserId));
            }

            var second = CreateOidcService(dbPath, CreateOidcPrincipal("oidc-disabled", issuer, subject, AppRole.ProjectUser));
            Assert.False(second.HasCurrentUser());
            Assert.Throws<UnauthorizedAccessException>(() => second.GetCurrentUser());
        }
        finally
        {
            DeleteFileQuietly(dbPath);
    ...
    public void SqliteStore_AddsExternalIdentityColumns_OnExistingDatabase()
    {
        ...
        try
        {
            ...
            Assert.Contains("is_disabled", columns);
        }
        finally
        {
            DeleteFileQuietly(dbPath);
    ...
    [Fact]
    public void GroupUsersQuery_SupportsFilterSortPagingAndAssignmentFlags()
    {
        using var ctx = TestAppContext.Create();
        ctx.LoginAsAdmin();

        var group = ctx.App.CreateGroup(new CreateGroupRequest { Name = "Ops Team", Description = "Operations" });
        var u1 = ctx.App.CreateUser(new CreateUserRequest
        {
            Username = "amy",
            DisplayName = "Amy Alpha",
            Role = AppRole.ProjectUser,
            IsLocalAccount = true,
            LocalPassword = "AmyAlpha123!"
        });
        var u2 = ctx.App.CreateUser(new CreateUserRequest
        {
            Username = "zoe",
            DisplayName = "Zoe Zebra",
            Role = AppRole.ProjectUser,
            IsLocalAccount = true,
            LocalPassword = "ZoeZebra123!"
        });
        var u3 = ctx.App.CreateUser(new CreateUserRequest
        {
            Username = "bob",
            DisplayName = "Bob Beta",
            Role = AppRole.ProjectUser,
            IsLocalAccount = true,
            LocalPassword = "BobBeta123!"
        });

        Assert.True(ctx.App.AssignUserToGroup(new AssignUserToGroupRequest { UserId = u2.UserId, GroupId = group.GroupId }));

        var page1 = ctx.App.GetGroupUsers(new GroupUserMembershipQuery
        {
            GroupId = group.GroupId,
            SortField = "displayName",
            SortDirection = SortDirection.Asc,
        ...
        var page2 = ctx.App.GetGroupUsers(new GroupUserMembershipQuery
        {
            GroupId = group.GroupId,
            SortField = "displayName",
            SortDirection = SortDirection.Asc,
        ...
        var combined = page1.Items.Concat(page2.Items).ToList();
        Assert.Contains(combined, x => x.Username == "zoe" && x.IsAssigned);
        Assert.Contains(combined, x => x.Username == "amy" && !x.IsAssigned);
        Assert.Contains(combined, x => x.Username == "bob" && !x.IsAssigned);

        var assignedOnly = ctx.App.GetGroupUsers(new GroupUserMembershipQuery
        {
            GroupId = group.GroupId,
            Assigned = true
        });

        Assert.Single(assignedOnly.Items);
        Assert.Equal("zoe", assignedOnly.Items[0].Username);

        var filtered = ctx.App.GetGroupUsers(new GroupUserMembershipQuery
        {
            GroupId = group.GroupId,
            FilterText = "beta"
        });
        ...
        Assert.Single(filtered.Items);
        Assert.Equal("bob", filtered.Items[0].Username);
    }
    ...
    private static SecureJournalAppService CreateService(
        IConfiguration configuration,
        IPrototypeDataStore store,
        PrototypeSharedState shared,
        NullLogger<SecureJournalAppService> logger,
        HttpContextAccessor? accessor = null,
        PrototypeSessionRegistry? sessions = null)
    {
        var checksum = new Sha256ChecksumService();
        var journalEncryptor = new JournalFieldEncryptor(
            EncryptionKeyParser.GetKeyBytes(configuration["Security:JournalEncryptionKey"], "journal"));
        var auditEncryptor = new PlaintextAuditFieldEncryptor();

        return new SecureJournalAppService(
            checksum,
            auditEncryptor,
            new JournalEntryRecordFactory(checksum, journalEncryptor),
            new AuditLogRecordFactory(auditEncryptor, checksum),
            new RecordViewMapper(journalEncryptor, auditEncryptor),
            new ExportContentFormatter(),
            store,
            shared,
            configuration,
            logger,
    ...
    private static SecureJournalAppService CreateOidcService(string dbPath, ClaimsPrincipal principal)
    {
